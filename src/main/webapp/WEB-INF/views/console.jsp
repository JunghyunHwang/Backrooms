...
<head>
  ...
  <script>
    $(function() {
      ...
      function bindFilterBtns(category, filter) {
        switch(category) {
          ...
          case "qna":
            ...
            $("#replyCompletedBtn").on("click",replyCompletedBtnHandler );
            break;
          default:
      ...
      function replyCompletedBtnHandler(event) {
        setActiveFilterBtn("qna", "completed");
        fetchAndDisplayBoardList("qna", 1, "completed");
      }
      ...
      function isImageIncluded(imageFileNameList) {
        if (imageFileNameList === undefined || imageFileNameList.length === 0) {
          return;
        }
      ...
      //###TODO
      function fetchAndDisplayBoardList(category, curPage, filterFromArgument, qnaCategoryFromArgument) {
        const params = new URLSearchParams(window.location.search);
        const filterFromUrl = params.get("filter") ?? "all";
        const filter = filterFromArgument === undefined ? filterFromUrl :  filterFromArgument
        let clientUrl = "${pageContext.request.contextPath}/console/board" + "?category="+category+"&curPage="+curPage+"&filter="+filter;
        let requestUrl = "${pageContext.request.contextPath}/console/board/" + category + "/list/" + curPage + "?filter=" + filter;
        console.log("qnaCategoryFromArgument: ", qnaCategoryFromArgument);
        const qnaCategoryFromUrl = params.get("qnaCategory") ?? "all";
        const qnaCategory =  qnaCategoryFromArgument ? qnaCategoryFromArgument : qnaCategoryFromUrl;
        if (category === "qna") {
          requestUrl = "${pageContext.request.contextPath}/console/board/qna/list/" + curPage + "?filter=" + filter + "&qnaCategory=" +  qnaCategory;
          clientUrl = "${pageContext.request.contextPath}/console/board" + "?category="+category+"&curPage="+curPage+"&filter="+filter + "&qnaCategory=" +  qnaCategory;
        }
        $.ajax({
          type : "get",
          url : requestUrl,
          dataType : "json", //text, json, html, xml
          success : function(data, status, xhr) {
            displayListData(category, data, qnaCategory);
      ...
      function displayListData(category, data, qnaCategory) {
        ...
        switch(category) {
          ...
          function createQnaCategoryHeader(qnaCategory) {
            let all = '<option value="all" >모든 문의</option>';
            let reservation = '<option value="reservation">예약 문의</option>';
            let room = '<option value="room">객실 문의</option>';
            let service = ' <option value="service">서비스 문의</option>';
            let payment = '<option value="payment">결제 문의</option>';
            let refund = '<option value="refund">취소 및 환불 문의</option>';
            let etc = '<option value="etc">기타</option>';

            switch (qnaCategory) {
              case "all":
                all = ' <option value="all" selected>모든 문의</option>';
                break;
              case "reservation":
                reservation = '<option value="reservation" selected>예약 문의</option>';
                break;
              case "room":
                room = '<option value="room" selected>객실 문의</option>';
                break;
              case "service":
                service = ' <option value="service" selected>서비스 문의</option>';
                break;
              case "payment":
                payment = '<option value="payment" selected>결제 문의</option>';
                break;
              case "refund":
                refund = '<option value="refund" selected>취소 및 환불 문의</option>';
                break;
              case "etc":
                etc = '<option value="etc" selected>기타</option>';
                break;
            }

            return  '<th scope="col" style="width: 50%;" class="text-start">' +
                    '<select id="qnaCategorySelectBox" class="form-select form-select-sm" style="width: auto;">' +
                    all +
                    reservation +
                    room +
                    service +
          ...
          case "qna": {
            ...
            console.log(createQnaCategoryHeader(qnaCategory))
            console.log("qnaCategory ## : ", qnaCategory); // all, reservation ,room, service, payment, cancleOrRefund, etc
            // createQnaCategoryHeader(qnaCategory)
            const qnaCategoryHeader = createQnaCategoryHeader(qnaCategory);
            tableHeadTr.innerHTML =
        ...
        if (category === "qna") {
          addChangeEventHandlerToSelectBox()
        }
      }
      ...
      //###TODO
      function addChangeEventHandlerToSelectBox() {
        // $("#qnaCategorySelectBox").on("change", function() {
        //   console.log("changed: ", this.value);
        // })
        document.querySelector("#qnaCategorySelectBox").addEventListener("change", function() {
          console.log("changed: ", this.value);
          const params = new URLSearchParams(window.location.search);
          const filter = params.get('filter') ?? "all";
          const curPage = params.get('curPage') ?? 1;

          fetchAndDisplayBoardList("qna", curPage, filter, this.value);
        });
      }
      ...
      function bindActiveMenu(category, filter) {
        $("#menu").on("click", function(event) {
          ...
          if (category == "notice") {
            ...
            fetchAndDisplayBoardList(category, 1, "all", "all");
          }
           ...
           <!-- Pagination area: Centered pagination with a fixed margin at the bottom -->
                ...
                <div class="row align-items-center">
                  ...
                  <div class="col-4 ms-2 d-flex justify-content-start gap-3">
                    ...
                    <button type="button" class="filter-btn btn btn-outline-primary" data-filter="completed" id="replyCompletedBtn">완료글</button>
                  </div>
                  <!-- Pagination in the center -->
                  <div class="col-4 d-flex justify-content-center">
                    <ul class="pagination flex-nowrap mb-0" id="pagination">
